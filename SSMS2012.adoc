...
`SQL CMD Mode` adds simple macros to the development environment.  It is also available from the command line.  See https://msdn.microsoft.com/en-us/library/ms162773.aspx[sqlcmd Utility].
...
Use `RAISERROR` to add information about a query's state during execution to the Messages pane.  When the query is executing.  Press kbd:[F6] to switch to Messages pane.
...
NOTE: This tends to conflict with <<SQL Command Mode>>.
